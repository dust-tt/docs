...
<Update label="July 24th, 2026" tags={["Added"]}>
## Claude Opus 5 Is Now the Default Opus Model in Dust

🎯 **What is it?**

Claude Opus 5, Anthropic's latest Opus model, is now available in Dust and is the default model for Opus-based workflows.

⚙️ **How does it work?**

New and existing workflows that use Dust's default Opus selection now use Claude Opus 5. You can also select Claude Opus 5 directly when building or editing an agent in the agent builder's model picker.

🚀 **How to access it?**

Open the agent builder, choose a model, and select **Claude Opus 5** from the Anthropic models. Claude Opus 5 is available to workspaces with access to Opus models.

⚠️ **Please note:** Claude Opus 5 is currently available through Anthropic's global endpoint. It is not yet available for workspaces with EU data residency.
...
The Computer is available to everyone, with one exception: Enterprise workspaces that were not part of the beta. If that's your case, you can opt in at any time, and every workspace will automatically receive the feature within the next 2 weeks. You can learn more in the [documentation](https://docs.dust.tt/docs/computer).
...
This is available now to everyone, no feature flag required. To get started, follow the connection steps in the [documentation](https://docs.dust.tt/docs/dust-mcp-server).
...
Pods are available to **everyone**: no feature flag, no opt-in required. Look for the **Pods** section in your Dust sidebar. Documentation is at [https://docs.dust.tt/docs/pods-overview](https://docs.dust.tt/docs/pods-overview).
...
Context Compaction is now available to all users. Watch for the context usage indicator in your input bar. When it appears, you'll be guided through the process automatically. Learn more in our [documentation](https://docs.dust.tt/docs/context-compaction).
...
Wake-ups are now available to all users. Ask your agent to "check back in \[time]" or "run this every \[schedule]" and it will use the wake-up tool automatically. For more details on how to configure wake-up schedules, check out the documentation: [https://docs.dust.tt/docs/wake-ups](https://docs.dust.tt/docs/wake-ups)
...
This feature is available to all workspaces. Check out our documentation to set up the integration: [https://docs.dust.tt/docs/gong-mcp](https://docs.dust.tt/docs/gong-mcp)
...
Steering is now available to everyone automatically. Start a conversation with any agent and try sending a follow-up message while it's working. For complete details and examples, visit the [full documentation](https://docs.dust.tt/docs/steering-conversations-that-keep-up-with-you).
...
## Email your agents directly at agent-name@dust.team
...
📖 Full documentation available here: [https://docs.dust.tt/docs/email-agents](https://docs.dust.tt/docs/email-agents)
...
Workspace admins can set up the Fathom MCP integration via OAuth in your workspace settings. Once configured, add the Fathom MCP to any agent's toolkit when building or editing an agent. Full setup instructions are available in our documentation: [https://docs.dust.tt/docs/fathom-mcp](https://docs.dust.tt/docs/fathom-mcp)
...
📚 **Learn more**: [Agent Builder Sidekick documentation](https://docs.dust.tt/docs/agent-builder-sidekick)
...
The upgrade is already live in your Dust workspace. Use the image generation feature as usual: the new capabilities are automatically available. For inspiration and detailed examples, check out our documentation: [https://docs.dust.tt/docs/image-generation](https://docs.dust.tt/docs/image-generation)
...
Skills are now available to all builders in your workspace. Check out our documentation to learn how to create and manage Skills: [https://docs.dust.tt/docs/skills](https://docs.dust.tt/docs/skills)
...
• New Gemini 3 Flash model added to the agent builder
• Fast performance with strong benchmark results
• Available for immediate use in agent configurations
...
• Messages are now automatically saved as drafts while you're composing them
• Unsent messages persist across page reloads and browser sessions
• Your drafts will be restored exactly where you left off
...
You can now search and attach files directly from the attachment button in the input bar for:
• Google Drive Tools
• Microsoft Drive Tools
• Notion Tools
...
• Added advanced search capabilities for GitHub MCP issues and pull requests
• Implemented robust pagination to handle large repositories
• Support for GitHub's advanced search syntax to filter and find specific items
...
• Programmatic usage is now tracked and billed separately from regular usage
• New dedicated 'API & Programmatic' section for admins to monitor usage and purchase credits
• Programmatic usage is blocked when credits run out, while human usage continues uninterrupted
• Free discovery credits provided monthly
• Enterprise users can opt for Pay-as-you-go billing
...
• Optimized the search (retrieval) engine for faster performance
• Search results now load significantly faster
...
• Claude Sonnet 4.5 now supports Structured Response Format with JSON schema
• Define a JSON schema in the advanced menu of the Agent Builder to enforce model output format
• Useful for workflows where agent output needs to be parsed and sent to other tools
...
User search now supports diacritics (accents) and provides better ranking of results.
• Search now works properly with accented characters in names
• Email addresses are now searchable with smart handling of username and domain
• Improved search available in Space user management and Agent editor management
...
Admins can now define and adjust approval settings for MCP tools directly from the admin interface:
• High stake: Agent always asks users to confirm before using the tool
• Low stake: Agent asks users to confirm, with option to 'never ask again'
• Never ask: Agent performs the action directly without confirmation
...
• New webhook provider for Linear integration
• Trigger agents automatically on Linear issues and project updates
• Configure webhooks directly in Spaces settings
...
• Service principals can now be restricted to specific sites only
• No longer requires Sites.Read.All permission for limited access scenarios
• Provides more fine-grained control over Microsoft connector permissions
...
• Agent mentions (+agent or \~agent) must now come immediately after @Dust
• Previous behavior allowed agent mentions anywhere in the message
...
You can now edit images directly within Dust:
• Upload images to the platform
• Adjust and modify images in-place
• Annotate images with built-in tools
• No need to switch to external image editing applications
...
• GPT-5.1 is now available as a model option in the agent builder
• This is an alternative to GPT-5, which remains available
• The new model offers improved performance characteristics
...
• Added a new 'Insights' tab in the Agent Builder
• View usage metrics and tool execution data
• Track feedback trends and latency performance
• Correlate all metrics with specific agent versions for performance tracking
...
• New built-in webhook provider for Fathom
• Automatically trigger agents when new Fathom transcripts are available
• Configure webhooks directly from your Space settings
...
You can now trigger your agents automatically with Zendesk events:
• New tickets
• New comments
• Status changes
• Configure webhooks directly in your Spaces
...
• Fetch tickets directly from Zendesk
• Pull ticket metrics for analysis
...
• Predictable behavior when receiving multiple workspace invitations, no more random redirects
• New selection page when you have multiple pending invitations, allowing you to choose which workspace to join
• Invitation tokens are now always respected, ensuring you land in the correct workspace
• Invitations now expire as expected
• Pending invitations are now visible on your profile page with quick join buttons
...
• Added native Jira webhook integration to trigger agents automatically
• Agents can now respond to Jira ticket updates in real-time
• Webhooks can be configured directly in Spaces settings
...
• Trigger agents automatically on GitHub pull requests and issues updates
• Configure webhooks directly in Spaces settings
• Integrates with GitHub MCP Server for complete automation workflows
...
• Agents can now be triggered by incoming webhooks
• Admins can set up webhook sources directly in the Spaces UI
• Webhook sources can be connected to any new or existing agent
...
• New option to automatically create empty Spaces when groups are provisioned
• Spaces are created alongside their corresponding groups
...
• Upgraded to Nano Banana, Google's latest image generation model
...
• When you don't @ mention a specific agent, your message now automatically goes to @dust
• If @dust is not activated in your workspace, we still recommend a list of suited agents
...
• The global agent can now execute SQL queries using the data warehouses tool
• Supports Snowflake and BigQuery connections
• Get instant answers to quantitative questions by querying structured data directly
...
• The Dust global agent can now store and recall information from your conversations
• Memory tool enables learning from interactions to provide personalized responses
• Preferences and context are remembered across conversations for more relevant assistance
...
The Dust global agent can now access Advanced Search (Data Source File System) in addition to traditional semantic search. This enables the agent to:
• Read documents in full
• Navigate data source structures
• Provide more precise answers with reduced hallucinations
...
• The Dust Google Sheets add-on is now officially published and available directly from the Google Workspace Marketplace
• No more manual script installations required
• Install and connect Dust to your spreadsheets in just a few clicks
...
• Improved user permissions mapping for private channels and direct messages
• Improved list channels functionality with reduced payload size to optimize LLM context usage
• Added 'get channels details' function for more granular channel information
• Added ability to schedule messages
• Added ability to add and remove reactions to messages
...
• Added tools to list, read, write, create, and clear data in Excel files
• Direct integration with SharePoint and OneDrive storage
• Simpler Excel workbook automation
...
• Added new Microsoft Teams MCP server with a full set of tools
• Search messages content across Teams
• List teams, users, channels, and chats
• View messages in channels
• Post messages to teams and chats
...
• Added search and retrieval capabilities for SharePoint and OneDrive files
• Added file upload functionality to SharePoint and OneDrive
• Added Word document editing capabilities
• Agents can now perform end-to-end document workflows across Microsoft platforms
...
• Interact with Dust assistants directly from Microsoft Teams
• Available in private chats with Dust or any Teams channel
• Install via feature flag for Microsoft ecosystem users
...
• Dust assistant can now automatically discover and access available tools in the Company Data space
• Tools are enabled dynamically on-demand during conversations
• Expands what the assistant can do without manual tool setup
...
• Deep Dive can now explore and query entire data warehouses (Snowflake, BigQuery) without pre-configuration
• Search for tables by name and examine schemas dynamically
• Run SQL queries across multiple tables in a single research session
• Support for large-scale data exploration (hundreds of tables, thousands of columns)
...
• Deep Dive sub-agents now receive AI-generated summaries (200-500 tokens) instead of full page content when browsing web pages
• Full page content is stored as accessible files for detailed analysis when needed
• Enables browsing 20+ pages instead of 2-3 pages within context limits
• Improves context efficiency by at least an order of magnitude for research tasks
...
• @deep-dive can now discover and call MCP tools dynamically during research
• Uses toolsets\_\_list to see available tool servers and selects relevant ones (GitHub, Salesforce, Monday, etc.)
• Spawns sub-agents with specific tools for focused tasks
• Extends capabilities on-demand based on task requirements
...
• All agents now have improved context management that preserves conversations across more turns
• When context window fills up, old tool outputs get pruned while keeping tool calls visible
• Maintains reasoning chain so agents remember what they've done
• Conversations stay coherent across more consecutive turns
• Agents don't lose context even in extended back-and-forth sessions
...
• Added a new Go Deep Tool that allows agents to hand off complex tasks for thorough analysis
• Tool is available by default in @Dust agent
• Can be added to any custom agent through the agent builder
• Enables specialized agents to use deep-dive capabilities while maintaining awareness of main agent instructions
...
• Added a new @deep-dive agent that conducts thorough research across company data, data warehouses, and the web
• Uses sub-agents to perform parallel research tasks for extended periods (10+ minutes)
• Explores topics iteratively before producing detailed answers
• Available for complex tasks requiring in-depth analysis and synthesis
...
• Added option to select 'agent responds in conversation' when using Run Agent MCP tool
• Agents can now post their answers directly in the conversation instead of running in background
• Available across all workspaces for any user
...
• Added exploratory search mode with new tools for systematic data navigation
• New `list` tool to browse folders and directory structures
• New `find` tool to search for documents by filename
• New `cat` tool to read complete documents instead of just chunks
• New `locate_in_tree` tool to view document hierarchies
• Updated `search` tool with folder-scoped searching capabilities
...
• Added tools to interact with Val Town's platform
• Users can now write and scale serverless JavaScript directly from Dust
• Build, deploy, and manage Val Town scripts
• Enables automation and custom workflows through Val Town's serverless JS platform
...
• Added support for the official GitLab Model Context Protocol (MCP) as a remote MCP
• Automatic app creation on GitLab when registering the MCP from Dust
• Available tools include: issue management, merge request operations, pipeline monitoring, and semantic code search
• Integration is now live for all users
...
• Agents can now get the current time
• Added ability for agents to wait/pause execution
• Agents can generate random numbers and dice rolls
• These utilities are available to all agents immediately
...
• New workspaces can no longer create or use Dust Apps as tools
• Existing workspaces with Dust Apps remain unaffected
• Dust Apps will be replaced by various tools including fetch capabilities
...
• Added support for Anthropic's new Claude 4.5 Haiku model
• Available for selection when creating or editing agents under Advanced → From Anthropic → Claude 4.5 Haiku
...
• Agents can now create interactive components (Frames) instead of static text responses
• Create live reports, dashboards, calculators, and data visualizations that users can explore in real time
• Interactive features include hover, filter, click, and dive functionality
• Frames update instantly when you ask your agent to make changes
• Share Frames internally or externally as live, interactive files with secure, token-gated links
• Available to all users with no feature flag required
...
• Progressive loading of conversations and agents lists
• Smooth conversation scrolling with virtualized message lists
• Optimized performance for conversations with thousands of messages
• Bigger touch targets for mobile-friendly interactions
• Fixed mobile layouts for agent builder and conversation views
• Smart keyboard handling that prevents unexpected pop-ups
• Proper iOS status bar colors and theming
• Touch-friendly interactions with disabled hover effects on mobile
• Resizable panels that work correctly on mobile screens
...
• Agents can now share documents directly in Slack channels
• Available for all agents with the Slack tool enabled
• Enables automated document distribution and workflow integration
...
• Record audio directly in conversations or attach audio files (up to \~1 hour)
• Automatic transcription with support for 29 spoken languages
• Intent detection to automatically select relevant agents based on voice commands
• Audio processing respects workspace region settings (EU/US models)
...
• Added OAuth authentication for personal or workspace credentials
• Enabled agents to create new pages directly in Confluence spaces
• Enabled agents to update existing pages in Confluence spaces
• Added write actions alongside the existing semantic search capabilities
...
• You can now select full datasources (not just individual files) directly from search results
• Available in Agent Builder, Knowledge section, and the input bar
• Enables adding all content from a datasource with a single click
...
• Long text content pasted into conversations is now automatically converted into file attachments
• Keeps conversations visually clean and uncluttered
• Handles lengthy content without disrupting message flow
...
• Actions like creating issues and posting PR comments now appear as coming from you instead of the dust-agent bot
• Personal attribution improves collaboration and makes audit trails clearer
• Available to all users with no feature flag required
...
• Agents can now access Google Drive documents in real-time using personal user credentials
• Read-only access with role-based permissions - users only see files they have access to
• No syncing or indexing required - documents are accessed on-demand
...
• Use Dust directly inside Microsoft Excel on both online and desktop versions
• Access Dust functionality directly within your Excel workflows
• Available at [office-addins.dust.tt](https://office-addins.dust.tt) with full documentation
...
• Create, read, update, and transition Jira issues directly from Dust
• Advanced search capabilities with custom fields and JQL query support
• Flexible authentication options with personal or workspace credentials
• Comment on issues and manage workflows
...
• Users can now add schedules to their agents in the Agent Builder. More info here [https://docs.dust.tt/docs/scheduling-your-agent-beta#/](https://docs.dust.tt/docs/scheduling-your-agent-beta#/)
...
• Added thumbs up, thumbs down, and arrow down reaction buttons to agent replies in Slack
• Users can now provide direct feedback on agent responses without leaving Slack
• Feedback reactions are consistent with the web application experience

![]()

![](https://files.readme.io/955d3416f0f64b1262d652530a3fde52c105336c3a0263101faa3ac65889944a-image.png)
...
• First available policy automatically retries tools if they are interrupted due to timeouts, deployments, or other issues\
• Tool developers can flag each tool with specific retry policies\
• Improves tool reliability and enables longer-running operations to resume after interruptions
...
• Added 'Used by' columns to the Space admin tools list\
• Admins can now instantly see which assistants are using which tools\
• Consistent 'Used by' UI across all admin interfaces
...
• Conversations now display clear indicators when you have unread messages\
• Makes it easier to track new activity across ongoing conversations\
• Helps prioritize which conversations need attention
...
• New simplified interface for building custom agents\
• Improved tool selection dialog with better discoverability\
• XML block formatting for structured instructions\
• Redesigned data source selection experience\
• More intuitive knowledge configuration\
• Better accessibility for new builders while maintaining advanced capabilities for power users
...
• Added 'blocking' parameter to Zapier integration\
• Allows instant retrieval of conversation ID without waiting for agent response\
• Enables separate polling for conversation results to avoid timeout issues

![](https://files.readme.io/17cfc095e808b3bbb32643525e8d8d80b7c3a963bdf5bb37ddc3d3e1619b4dd0-Screenshot_2025-09-01_at_14.19.31.png)
...
• Migrated table data storage from SQL to individual GCS files\
• Significantly improved data upsert performance\
• Reduced storage footprint by over 80%
...
• Added `--agent` flag for specifying agent name at launch\
• New `/attach` and `/clear-files` commands for native file management\
• Implemented caching system for faster CLI startup\
• Added `dust cache:clear` command for cache management
...
• Automatic handling of access to underlying tables when a view is selected\
...
• Added support for client-side rendered pages and JavaScript execution\
• Integrated PDF and Google Docs parsing capabilities\
• Improved content extraction from modern websites\
• Implemented residential proxy rotation for better reliability
...
• Added support for key pair authentication in the Snowflake connector\
...
• Added ability to sync custom SOQL queries as data source documents\
• Documents from SOQL queries are now available through the Search tool
...
- Doc is available [here](https://docs.dust.tt/docs/remote-mcp-server)
...
• New script to import issues from Linear into Dust\
...
• Added a new notice screen explaining the account merge process when signing in with different authentication methods\
• Automatic redirection to merge flow when duplicate email accounts are detected\
• Accounts are now consolidated based on the oldest identity
...
• Added ability to search and attach documents, tables, and folders from Knowledge directly in the input bar\
• Added support for pasting document URLs in the input bar to attach them
...
• Added Service Principal (service account) authentication method as an alternative to OAuth\
• Enables automated, non-interactive authentication with Microsoft services
...
[Read more](https://docs.dust.tt/docs/gong-connection)
...
• Added label-based filtering capabilities to Search, Include, and Extract data source tools\
• New documents in Connections now automatically capture labels
...
[Read more](https://docs.dust.tt/docs/bigquery)
...
[Read more](https://docs.dust.tt/docs/chrome-extension)
...
- Documentation: [https://docs.dust.tt/docs/n8n](https://docs.dust.tt/docs/n8n)
</Update>
...
• Added O3-mini model with two modes: normal and high-reasoning\
• Available in the builder as main assistant and reasoning model\
• New 'O3 Mini' global agent using O3-mini high-reasoning mode
...
• Added a new tool for Assistants to invoke reasoning models (R1 or O1)\
• Enables better handling of complex problems while maintaining fast responses for simpler queries\
• Integrates with existing Assistant tools and instructions\
• Supports both DeepSeek R and OpenAI O1 models
...
• Dust is now HIPAA compliant\
• Documentation can be found on the security page and on Dusts' trust center
...
• Added feedback functionality in conversations\
• Added support for EU region\
• Added URL blacklisting with path-level control\
• Improved SSO workspace integration\
...
Added support for multiple new file formats:\
• Programming languages: Python, C/C++, C#, Java, PHP, Ruby, SQL, Swift, Rust, Go, Kotlin, Scala, Groovy, Perl\
...
Conversations are now automatically sorted by most recent activity:\
• When posting a message in a conversation, it moves to the top of the list\
• Most recently used conversations appear first
...
• Added DeepSeek V3, a new large language model, to the platform\
• Model can be used in Dust Apps, as a global agent, and in custom assistants\
• Known for high performance on benchmarks, fast response times, and efficient processing\
• Enabling DeepSeek on a Workspace requires reaching out to Dust
...
• Improved assistant loading time for quicker auto-complete availability\
• Simplified input bar animation for smoother transitions
...
• Added detailed documentation to help deploy Dust within organizations\
• Introduced resources including webinar registration and community access\
• Created step-by-step guidelines for successful enterprise implementation

[Read more](https://docs.dust.tt/docs/dust-enterprise-rollout-guide)
...
- Check out the docs at [https://docs.dust.tt/docs/zendesk-connection](https://docs.dust.tt/docs/zendesk-connection)
</Update>

<Update label="December 5th, 2024" tags={["Added"]}>
## 💾 Prompt version history

• Added ability to recover previous assistant instructions\
• Each version of a prompt is now saved and accessible
...
• Official Dust Chrome extension released on Chrome Web Store\
• Share webpage content directly in Dust conversations\
• Access Dust features while browsing
...
[Read more](https://docs.dust.tt/docs/raycast)
...
• Visualization feature automatically enabled for new assistants\
• Web search capability added to raw model assistants (except anthropic models)\
• Visualization support added to raw model assistants
...
- [Documentation](https://docs.dust.tt/docs/data)
</Update>

<Update label="October 21st, 2024" tags={["Added"]}>
## 🌨️ Snowflake Integration: Connect and Query Your Data

• Add Snowflake data warehouses as connections in Dust\
• Add Snowflake tables to vaults\
• Query Snowflake tables using Tables Query feature in assistants and Dust apps
...
• Query results are accessible by the model as a file\
• Supports larger datasets without manual data input\
• Allows retrieval of more than 128 rows from tables\
• Enables generation of charts with numerous data points directly from database
...
➡️ Connect Data to Dust: [https://docs.dust.tt/docs/connections](https://docs.dust.tt/docs/connections)\
➡️ Manage Data on Dust (Company Data vs Vaults): [https://docs.dust.tt/docs/data](https://docs.dust.tt/docs/data)\
➡️ Adding data to a vault as an admin: [https://docs.dust.tt/docs/data#how-to-add-data-to-a-vault](https://docs.dust.tt/docs/data#how-to-add-data-to-a-vault)
...
• Expanded character support for folder names\
• Now accepts any valid UTF-8 character\
• Improved user experience in folder creation and management
...
• Dust is now available as an integration on Make.com\
• Users can incorporate Dust functionalities into their Make.com scenarios\
• Enables creation of automated workflows using Dust capabilities\
• Documentation available here: [Make.com: Trigger Dust assistants from any scenario](https://docs.dust.tt/docs/make-dust-integration)
...
• Enables indexing of various document types including docs, spreadsheets, PowerPoint presentations, and PDFs from Microsoft drives\
...
• Implemented Single Sign-On (SSO) support for Microsoft Entra ID\
• Enabled upon request for Enterprise customers
...
• Added support for the new GPT4o mini model\
• Users can now select and utilize this model when creating assistants
...
• Integrated support for Anthropic's vision models\
• All three Anthropic models now support vision capabilities\
...
• Added drag-and-drop functionality for files in the conversation view\
• Supported file formats include JPEG, PNG, and PDF\
• Simplifies sharing files within conversations
...
• Integrated Vision capability for OpenAI's gpt-4 and gpt-4-turbo models/assistants\
• Supports JPEG and PNG image formats\
• Available for all users to explore and use
...
• Introduced downloadable monthly reports as a zip file\
• Reports include four CSV files:
...
• Added support for LaTeX equations in agent messages\
• Useful for mathematical and scientific communication
