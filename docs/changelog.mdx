...
<Update label="September 9th, 2026" tags={["Added"]}>
## databricks: [external] Databricks official MCPs: SQL and Genie

Databricks official managed MCPs are available in Dust as 2 preset tools: SQL and Genie. This allows Dust agent to discuss with Databricks' semantic layer without the hassle of configuring a custom MCP. On Databricks' side, the MCPs are managed entirely by Databricks, and are bespoke to the user's instance.

Available to all users with no feature flag.
