...
Auto models (**Basic**, **Standard**, and **Premium**) now skip models affected by detected provider outages and use the next available candidate. The model picker shows the temporary replacement. No setup is required. If you selected a specific model, Dust warns you but keeps your choice.

If a provider error causes a message to fail on a specific model, you can retry with the corresponding auto-model level. Dust does not switch models mid-run. Review completed actions before retrying. Read about [provider outage handling](/docs/user-documentation/agents/model-selection#provider-outages-and-automatic-fallback).
