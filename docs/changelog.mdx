...
<Update label="September 23rd, 2026" tags={["Added"]} rss={{ title: "Qobra remote MCP" }}>
## Qobra remote MCP

Qobra is now available as a default remote MCP server in Dust’s MCP catalog, with separate EU and US regional entries. Admins can add Qobra from Spaces > Tools and connect it through OAuth, letting agents read sales compensation data — commission plans, quotas, statements, payouts, reports, and dashboards — scoped to the connected user's own Qobra permissions.

Available in all workspaces. Qobra must be enabled for the company by the customer's own Qobra admin, from Your account > Settings > AI > MCP in Qobra, before anyone can connect.
