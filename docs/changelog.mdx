...
## Inline tool and knowledge references in the composer

You can now add knowledge and tool references directly in the composer, so they appear inline with the rest of your agent context. This makes it easier to build prompts that clearly show which sources and capabilities an agent should use. Available to everyone now.
...
Dust now pauses an agent message after it consumes 200 LLM credits and asks whether you want to continue or stop. Nothing is lost when the circuit breaker appears, so you can review the task before allowing it to run further.
