...
<Update label="September 15th, 2026" tags={["Added"]}>
## StackOne MCP as a remote MCP preset

StackOne MCP is now available as a one-click remote MCP preset in Dust. Admins can configure StackOne once in Spaces > Tools, giving builders access to HR, ATS, CRM, and other enterprise SaaS tools through a single managed connection instead of setting up each system separately.

Available in all workspaces with a StackOne account.
