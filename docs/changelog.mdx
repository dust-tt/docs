...
<Update label="September 9th, 2026" tags={["Added"]}>
  ## DeepSeek V4 Pro refresh

  DeepSeek V4 Pro is now served through the supported `deepseek-v4-pro-0813` model on Fireworks. The previous preview model has been retired, and existing agent configurations are migrated to the replacement without changing their effective reasoning level.
</Update>
