...
<Update label="September 14th, 2026" tags={["Improved"]}>
## Your triggers, 50% cheaper

Schedules, wakeups, and other triggers now cost 50% less when they use an OpenAI model. This includes Basic and Standard agents, which currently default to an OpenAI model for these asynchronous runs. They use the lower-priority queue, with no perceptible latency impact expected, though performance may be marginally affected. Generally available.
