---
title: "Overview"
description: "Pods are shared workspaces where humans and agents collaborate around conversations, tasks, and files."
---
...
Pods with Apps enabled include an **Apps** tab in the Pod navigation. Open it to browse the apps discovered in the Pod.

An app is a folder at the Pod root that groups a Frame with the functions and databases behind it. The Apps tab lists the app's Frames, published functions, and databases together, so you can open a Frame and see the artifacts that support it in one place. Apps are derived from the files and published artifacts in the Pod rather than stored as separate records.
