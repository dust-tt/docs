...
New skill versions default to `editors` availability. They are therefore hidden from the input bar and Agent Builder for non-editors until an administrator or a user with the required permission broadens their availability. To let members or agents discover a skill, choose a broader availability.
