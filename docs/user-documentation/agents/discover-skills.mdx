...
description: "Let agents find and enable relevant workspace skills during a conversation."
---
...
Discover Skills only sees workspace skills marked as discoverable. Mark a skill as discoverable if you want agents with this capability to find and enable it. Skills that are not discoverable do not appear in the available skill list.nnNew skill versions default to `editors` availability. They are therefore hidden from the input bar and Agent Builder for non-editors until an administrator or a user with the required permission broadens their availability. To let members or agents discover a skill, choose a broader availability.
