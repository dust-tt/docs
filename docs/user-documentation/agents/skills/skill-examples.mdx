...
## Skills: rights and publication states
...
| **Use a skill** | Access to the skill according to its publication state |

Skills have three publication states:

- **Unpublished**: Available according to the workspace's unpublished-resource policy; not listed in the general skill directory.
- **Published**: Visible and usable by the audience defined by the skill and workspace access rules.
- **Discoverable**: Surfaced for broader discovery (e.g. via the Discover Skills capability) according to the workspace publication policy.

<Info>
**API note:** The Dust API exposes skill statuses `active`, `archived`, and `suggested`. These are internal lifecycle statuses and are distinct from the publication state described above.
</Info>
