...
- The **model picker in the conversation input bar** changes the model for your next message in that conversation, without editing the agent.
- **Advanced > Model selection** in the agent builder sets the model an agent uses by default.
...
1. **A model picker in the input bar.** Click the model icon to choose a model for any conversation, with any agent, including the default `@dust` agent.
...
3. **The global model agents were retired.** They no longer appear in your agent list. Model choice now lives in the model picker.

## Were the models removed?

No. The models that powered the retired global model agents are still available through the model picker when your plan and administrator settings allow them. The retired agents were wrappers around a model with access to internet search, rather than full agents with their own instructions, tools, or company knowledge.

## How to use a specific model now

- **For one conversation:** Click the model icon in the input bar, choose **More models**, and select the model you want. You can also type `/` in the input bar and select **Pick model**.
...
- Custom agents where the builder did not select a specific model previously followed the global `@dust` agent's model. They now run on **Standard**.
...
- The model picker is available in the conversation input bar and Agent Builder when the feature is enabled for your workspace.
...
No. A pick in the input bar applies to your next message in that conversation only. The agent's own configuration is untouched, and no other member sees a change. To change an agent for everyone, edit it in Agent Builder.
...
Yes. In Agent Builder, open **Advanced**, then **Model selection**, and choose a specific model. That agent then uses that model for every member and run, subject to plan and administrator access rules.
...
**Slack and triggers:** A pick in the input bar is scoped to that conversation in the Dust web app. An agent that runs from Slack or a trigger uses its configured model.
