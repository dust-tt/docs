...
- _Exclude specific content_ : Filter out pages with certain labels (e.g., "deprecated").
- _Include specific content_ : Only include pages with specific labels (e.g., from a particular author).
- _In-conversation filtering_ : Enable dynamic filtering based on conversation context. This requires exact tag matching. For example, you could ask "What projects currently have the label 'status:review'?".
...
1. Understand the user question (e.g. "_what should I prepare for a parental leave?_") and search for the right articles and the right chunks inside of the articles that will actually have the answer.
...
Agents can also gain filesystem-level access to your data sources, in addition to semantic search: listing folders, browsing directory structures, and reading specific files by their exact path. This complements semantic search by allowing agents to navigate your data organization directly (see [Find in data sources](/docs/user-documentation/agents/knowledge) on the Knowledge page).

This is no longer a manual "Search > Advanced settings" toggle. For **SharePoint via Content search, folder and path navigation is enabled by default for new knowledge configurations.**

<Warning>
  Configurations created **before** this change do not gain folder navigation retroactively. To pick it up, remove and re-add the SharePoint Content search knowledge with the same sources — but note this drops the configuration's existing must-have / must-not-have label filters (see warning above).
</Warning>
