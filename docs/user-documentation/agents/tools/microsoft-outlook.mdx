...
2. Review the permissions. Required permissions are pre-selected and cannot be
   removed; optional ones (shared mailboxes, contacts) can be unchecked.
...
After admin setup, Outlook tools are added to an agent by selecting them in the
agent builder. They require no configuration. Each tool can be set to one of
four stake levels: high (always ask for confirmation), medium (let users save
a confirmation for a specific input), low (users can disable confirmations
entirely), or never ask (runs automatically).
...
Most mail tools accept an optional shared mailbox address, provided the shared
mail permissions were granted at setup. This does not apply to the contact
tools (Get Contacts, Create Contact, Update Contact), which always operate on
the signed-in user's own contacts.
...
### "This tool requires personal authentication"

The tool is installed with personal credentials and the user has not connected
their Microsoft account yet, or their connection was invalidated. The user
should open the tool and click **Connect**, then complete the Microsoft consent
screen. A workspace-level connection does not cover personal tool calls.

### An agent says it cannot perform an action it should support

Check that the specific tool is enabled on the agent. Tools are selected
individually in the agent builder, so an agent with only Get Messages cannot
move a message, and may explain the failure incorrectly. Compare the agent's
tool list against the tables above.

### A tool worked before and now fails, or a permission seems missing

Dust occasionally adds scopes when shipping new tools. Connections created
before that change keep their original grant and need to be refreshed: the user
reconnects the tool, and an Entra admin may need to consent again for the added
permissions.

### Users are blocked on approval prompts

Grant permissions at admin level. Go to the Azure admin portal, then
[Enterprise applications](https://portal.azure.com/#view/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/~/AppAppsPreview),
then **Dust - Tools**, then **Permissions**, and click **Grant admin consent**.

**Still not working?** Try this alternative path:

1. Go to [Access requests](https://portal.azure.com/#view/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/~/AccessRequests).
2. Click **All (preview)**.
3. Click the **Dust - Tools** app.
4. Click **Review permissions and consent** in the right panel.

## Limitations

- **Email categories and tags.** Agents cannot set or read Outlook categories.
- **Deleting or archiving messages.** Messages can be moved to any folder,
  including a folder named Archive, but there is no dedicated delete tool.
- **Rules and mailbox settings management.** Agents cannot create or edit inbox
  rules, or change mailbox settings.
