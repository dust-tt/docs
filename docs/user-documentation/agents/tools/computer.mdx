...
Computer can extract text from text-based PDFs and use the PDF skill to process scanned or image-only PDFs. For scans, it renders each page and runs OCR to recognize the text, supporting English, French, and mixed English/French documents. It processes scans one page at a time, preserves page references, and reports pages where recognition fails or text remains illegible.
