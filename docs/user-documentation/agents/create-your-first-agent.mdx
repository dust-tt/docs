...
Note : Only Admins and Managers can create new tags [here is how](/docs/user-documentation/admins/agents-management). Other team members can select from existing tags

## Step 6: Set access and share your agent

An agent's sharing is controlled by two settings:

- **Availability**: determines who can access and use the agent.
  - **Editors**: the agent is available only to its editors.
  - **Workspace users**: the agent is available to all workspace members (or a subset defined by group access rules).
- **Agent Discoverability toggle** (ON/OFF): when turned on, the agent is surfaced in the workspace's agent directory so members can discover it.

Changing an agent's availability or discoverability requires the **`Publish agents`** governance capability. Editing the agent's content requires being an editor — having publish rights does not grant editing rights.
