...
<Info>
Creating an agent requires the **`Create agents`** governance capability, which is set to **Everyone** by default. Your workspace admin may restrict this to specific groups. See [Admin governance](/docs/user-documentation/admins/users-and-permissions-management/admin-governance) for details.
</Info>
...
Note : Only Super Admins and Managers can create new tags [here is how](/docs/user-documentation/admins/agents-management). Other team members can select from existing tags

## Step 6: Set access and share your agent

Agents have three publication states:

- **Unpublished**: The agent is available according to your workspace's unpublished-resource policy. It is not listed in the general agent directory but is not necessarily confidential.
- **Published**: The agent is visible and usable by the audience defined by the agent and workspace access rules.
- **Discoverable**: The agent is surfaced for broader discovery according to the workspace publication policy.

Changing an agent's publication state requires the **`Publish agents`** governance capability. Editing the agent's content requires being an editor — having publish rights does not grant editing rights.

You can define who will be able to edit the agent by adding users to the **"Editors" section**.
