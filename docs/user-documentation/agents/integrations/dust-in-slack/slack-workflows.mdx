...
2. **Whitelist your Slack workflow with the Dust team.** Dust must whitelist your Slack workflow before a Dust agent can reply to messages posted by the workflow Slackbot. Until this is done, the agent will not answer.

   <Warning>
   **Contact Dust support to get your workflow whitelisted**

   Email `support@dust.tt` with the template below. We cannot whitelist a workflow without these three pieces of information, so sending them all at once avoids any back-and-forth.
