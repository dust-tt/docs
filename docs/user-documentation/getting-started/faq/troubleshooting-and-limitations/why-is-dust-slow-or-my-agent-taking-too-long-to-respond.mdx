...
Check the model picker for an instability warning. Auto models (**Basic**, **Standard**, and **Premium**) skip degraded models for new messages. If you selected a specific model, you can choose an auto model or another available model instead.

If a message fails, follow the retry option shown in the error message and review any completed actions first. See [Provider outages and automatic fallback](/docs/user-documentation/agents/model-selection#provider-outages-and-automatic-fallback) for details.
