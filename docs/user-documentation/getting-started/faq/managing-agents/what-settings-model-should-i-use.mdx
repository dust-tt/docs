...
Auto models also skip degraded models for new messages during detected provider outages. A pinned model stays selected even if it is unstable, so choose an auto model if you want Dust to handle fallback. See [Provider outages and automatic fallback](/docs/user-documentation/agents/model-selection#provider-outages-and-automatic-fallback).
