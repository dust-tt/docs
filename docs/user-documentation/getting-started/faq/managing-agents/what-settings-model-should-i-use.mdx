...
Pick a specific model when you need the agent to use that model rather than follow an auto-model list. Pinning a model does not guarantee identical responses.

Auto models also handle fallback for new messages during detected provider outages. Pinned models do not switch automatically, even when unstable. See [Provider outages and automatic fallback](/docs/user-documentation/agents/model-selection#provider-outages-and-automatic-fallback).
