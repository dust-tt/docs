...
You can manage your subscription by going to **Workspace & Governance > Billing**. Access requires the **Super Admin** role or the **`Manage billing and security`** capability.
...
You can manage your subscription from **Workspace & Governance > Billing**. From there, you can upgrade your plan, update your payment details, and access invoices. Access requires the **Super Admin** role or the **`Manage billing and security`** capability.
...
The member with the **Super Admin** role who was assigned that role the earliest. This will typically be the person who initially set up the workspace, unless their role was changed later.

<Info>
**Legacy role aliases**: during the migration period, the API continues to accept the legacy `admin` role value as an alias for `super_admin`. If you set a member's role via the API using `admin`, they will be treated as a Super Admin.
</Info>

If you'd like a different user to remain, ensure the intended user holds the Super Admin role before your subscription ends.
...
Go to **Workspace & Governance > Billing** and resubscribe. Upon reactivating, all previously limited or inaccessible features and data will be restored, including access to multiple users, connections, data sources, and custom agents.
