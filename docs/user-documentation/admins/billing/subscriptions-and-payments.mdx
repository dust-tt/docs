...
You can manage your subscription by going to **Billing**. Access requires the **admin** role or the **`Billing access`** capability.
...
- **From the free option**: start with a free workspace and upgrade any member's seat to Pro or Max from **Workspace > Usage**. This triggers the checkout flow.
...
You can manage your subscription from **Billing**. From there, you can upgrade your plan, update your payment details, and access invoices. Access requires the **admin** role or the **`Billing access`** capability.
...
The member with the **`admin`** role who was assigned that role the earliest. This will typically be the person who initially set up the workspace, unless their role was changed later.

<Info>
**Legacy role aliases**: during the migration period, the API continues to accept the legacy `admin` role value, which maps to the Admin role. The legacy `user` value maps to Member. The `builder` role has been removed; affected API keys receive the **Builders** group (granted Create agents and Create skills capabilities) instead.
</Info>

If you'd like a different user to remain, ensure the intended user holds the `admin` role before your subscription ends.
...
Go to **Billing** and resubscribe. Upon reactivating, all previously limited or inaccessible features and data will be restored, including access to multiple users, connections, data sources, and custom agents.
...
_If you have any further questions about your Dust subscription, don't hesitate to contact us at [support@dust.tt](mailto:support@dust.tt)._
