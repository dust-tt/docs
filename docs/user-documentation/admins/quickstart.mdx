...
<Note>
Creating agents is available to all workspace members by default (governed by the `Create agents` capability). Publishing agents workspace-wide requires the `Publish agents` capability. Both capabilities can be restricted to specific groups by a Admin under **Admin > Workspace & Governance**.
</Note>
