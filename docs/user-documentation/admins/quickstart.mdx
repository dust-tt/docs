...
You can use Dust without data sources, but Dust is most useful when connected to your knowledge bases. **Managing data sources and connections requires the Admin role.** Workspace admins can make data available for all members of the workspace.
...
<Note>
Creating agents is available to all workspace members by default (governed by the `Create agents` capability). Publishing agents workspace-wide requires the `Publish agents` capability. Both capabilities can be restricted to specific groups by a workspace admin under **Admin > Settings & Governance**.
</Note>
...
**Settings & Governance** *(Admin only)*: Configure SSO, governance capability modes, feature toggles (Frames, Audit Logs), billing, and security settings under **Admin > Settings & Governance**.
