...
You can use Dust without data sources, but Dust is most useful when connected to your knowledge bases. **Managing data sources and connections requires the Super Admin role.** Super Admins can make data available for all members of the workspace.
...
**How to add my first connection** *(requires Super Admin role)*
...
<Note>
Creating agents is available to all workspace members by default (governed by the `Create agents` capability). Publishing agents workspace-wide requires the `Publish agents` capability. Both capabilities can be restricted to specific groups by a Super Admin under **Admin > Workspace & Governance**.
</Note>
...
1. [Install and authorize the Dust app in Slack](https://docs.dust.tt/docs/slack). *(Workspace-level Slack configuration requires the Super Admin role.)*
...
**Workspace & Governance** *(Super Admin only)*: Configure SSO, governance capability modes, feature toggles (Frames, Audit Logs), billing, and security settings under **Admin > Workspace & Governance**.
