...
**Setup**: An Admin, or a group member granted the corresponding administration capability under Workspace & Governance, provides credentials during tool setup
...
**Setup**: An Admin (or group member with the administration capability under Workspace & Governance) completes the initial OAuth flow, then each user authenticates individually
...
<Info>
**Even with personal credentials, an Admin (or group member with the administration capability under Workspace & Governance) must complete the initial OAuth flow to verify the connection works and gather necessary metadata (like Slack team ID, Salesforce instance URL, etc.) that will be reused for individual user connections.**
...
Complete the Admin (or administration group member) OAuth flow to:
...
Re-authenticating as needed (Admin or group member with the administration capability OAuth flow required for personal credentials)

*Note that changing from shared to personal credentials will require an Admin (or group member with the administration capability under Workspace & Governance) to complete the initial OAuth flow, followed by individual user authentication.*
