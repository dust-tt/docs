...
**Setup**: A Super Admin, or a group member granted the corresponding administration capability under Workspace & Governance, provides credentials during tool setup
...
**Setup**: A Super Admin (or group member with the administration capability under Workspace & Governance) completes the initial OAuth flow, then each user authenticates individually
...
<Info>
**Even with personal credentials, a Super Admin (or group member with the administration capability under Workspace & Governance) must complete the initial OAuth flow to verify the connection works and gather necessary metadata (like Slack team ID, Salesforce instance URL, etc.) that will be reused for individual user connections.**
...
Complete the Super Admin (or administration group member) OAuth flow to:
...
Personal credentials: More secure attribution but requires both Super Admin setup and individual user authentication\
...
Re-authenticating as needed (Super Admin or group member with the administration capability OAuth flow required for personal credentials)

*Note that changing from shared to personal credentials will require a Super Admin (or group member with the administration capability under Workspace & Governance) to complete the initial OAuth flow, followed by individual user authentication.*
...
Go to your Profile (avatar bottom-left) > Profile > Tools & Triggers
