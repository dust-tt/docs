...
Workspace admins can configure two areas:
...
Computer does not have open internet access by default. The Network section lets workspace admins decide which domains are trusted for the workspace.
...
The **Agent-requested domains** toggle controls whether agents can ask users to approve additional domains during a conversation. This toggle is managed by workspace admins.

When this setting is disabled, agents cannot request new domains. They must rely on the workspace allowed domains configured by workspace admins.
...
<Warning>
When Agent-requested domains is enabled, a non-admin user can grant temporary network access to a domain that admins did not preapprove. Enable it when you want users to unblock one-off API or website access during a conversation. Keep it disabled when every allowed domain should be reviewed by a workspace admin first.
</Warning>
...
Only workspace admins can add, update, or delete environment variables and HTTPS secrets.
...
Only workspace admins can add, replace, or delete HTTPS secrets.
