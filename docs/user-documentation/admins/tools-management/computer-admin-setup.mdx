---
title: "Computer admin setup"
---


This page explains how Super Admins (or a group granted the Computer administration capability under Workspace & Governance) can configure Computer. For an overview of what Computer does for users and builders, see [Computer](/docs/user-documentation/agents/tools/computer).

The Computer admin page controls workspace-level network access and environment variables for Computer. Only Super Admins, or a group granted the Computer administration capability under Workspace & Governance, can manage these settings.
...
Super Admins, or group members granted the Computer administration capability, can configure two areas:

* **Network access**, which controls which websites and APIs Computer can contact — including the workspace allowlist of trusted domains and whether users can request additional domains during a conversation.
* **Environment variables**, which provide configuration values or HTTPS secrets to Computer.

## Configure network access

Computer does not have open internet access by default. The Network section lets Super Admins (or group members with the Computer administration capability) decide which domains are trusted for the workspace.
...
The **Agent-requested domains** toggle controls whether agents can ask users to approve additional domains during a conversation. This toggle is managed by Super Admins, or a group granted the Computer administration capability under Workspace & Governance.

When this setting is disabled, agents cannot request new domains. They must rely on the workspace allowed domains configured by the Super Admin or administration group.
...
<Warning>
When Agent-requested domains is enabled, a non-admin user can grant temporary network access to a domain that admins did not preapprove. Enable it when you want users to unblock one-off API or website access during a conversation. Keep it disabled when every allowed domain should be reviewed by a Super Admin first.
</Warning>
...
Only Super Admins, or a group granted the Computer administration capability under Workspace & Governance, can add, update, or delete environment variables and HTTPS secrets.
...
Only Super Admins, or a group granted the Computer administration capability under Workspace & Governance, can add, replace, or delete HTTPS secrets.
...
Make sure Computer administration is enabled for the workspace and that you are a Super Admin or a member of a group granted the Computer administration capability under Workspace & Governance.
