...
Managing remote MCP servers is reserved for **Admins** by default, but can be delegated to specific groups under Workspace & Governance. To add a Remote MCP Server, navigate to Spaces > Tools. Here, you'll find an **Add Tool** button.

<Info>
Adding, removing, configuring, and changing tool approval settings for remote MCP servers are workspace-level governance actions performed by Admins, or a group granted the manage remote MCP servers capability under Workspace & Governance. This is distinct from ordinary agent editing: any user with the `Create agents` capability can select an already-configured tool and add it to their agent, without needing administration rights.
</Info>
