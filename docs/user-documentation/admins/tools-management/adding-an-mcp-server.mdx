...
1. In Github : create an OAuth app from the [developer settings](https://github.com/settings/developers) page
   ...
   2. Add all three Dust redirect URLs to the OAuth app:
...
Register all three callback URLs. `https://dust.tt` is the US endpoint, `https://eu.dust.tt` is the EU endpoint, and `https://app.dust.tt` is the global endpoint that will replace the two regional endpoints.
