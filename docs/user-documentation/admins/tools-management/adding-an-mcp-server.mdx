...
Managing remote MCP servers is reserved for **workspace admins**. To add a Remote MCP Server, navigate to Spaces > Tools. Here, you'll find an **Add Tool** button.

<Info>
Adding, removing, configuring, and changing tool approval settings for remote MCP servers are workspace-level governance actions performed by workspace admins. This is distinct from ordinary agent editing: any user with the `Create agents` capability can select an already-configured tool and add it to their agent, without needing administration rights.
</Info>
