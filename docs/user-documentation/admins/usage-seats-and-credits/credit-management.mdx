...
The workspace credit pool is managed by **Super Admins** and members with the **`Manage billing and security`** capability:

* **Business plan**: they can purchase top-up credit packages as needed.
...
By default, all members with workspace pool access can draw from it without individual limit. **Super Admins** and members with the **`Manage billing and security`** capability can configure a **personal spend limit** for each member to cap how much of the shared pool they can consume.
...
* Super Admins and members with the `Manage billing and security` capability can set a workspace-wide default limit. When the default is updated, only members currently on the previous default are affected.
...
**Super Admins** and members with the **`Manage billing and security`** capability can configure a monthly cap on programmatic credit consumption from **Admin > Usage**. An email notification is sent when 80% of the cap is reached.

## Tracking usage

**Super Admins** and members with the **`Manage billing and security`** capability can manage credit settings—including the workspace pool, spending limits, and programmatic caps—from **Admin > Usage**. **Managers** can also access this page to view usage data, edit member seat types, and review upgrade requests.

## Workspace credit pool

At the top of the page, the current workspace credit pool balance is displayed (credits consumed vs. total available). Super Admins and members with the `Manage billing and security` capability on Business plans can purchase top-up packages directly from this page using the **Top up** button.

## Settings

<HTMLBlock>{`
<div class="wistia_responsive_padding" style="padding:65.42% 0 0 0;position:relative;"><div class="wistia_responsive_wrapper" style="height:100%;left:0;position:absolute;top:0;width:100%;"></div></div>
...
* **Upgrade request**: when enabled, members who reach their credit limit can request an upgrade instead of being immediately blocked. Super Admins and Managers can review and act on those requests from the Requests tab in the Members section.
...
The Members section lists all workspace members with their usage details. **Super Admins** and **Managers** can:
...
* Use the **···** menu on each member row to change their seat type individually.
* Review pending upgrade requests in the **Requests** tab.

In addition, **Super Admins** and members with the **`Manage billing and security`** capability can adjust a member's personal spend limit from the same **···** menu.

For a broader view of how your team uses Dust (agent activity, conversation trends, and adoption over time), see [Workspace analytics](/docs/user-documentation/admins/workspace-analytics).
