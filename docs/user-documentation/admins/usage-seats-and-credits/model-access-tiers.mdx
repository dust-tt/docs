...
Members can pick the model an agent runs on from the conversation input bar and Agent Builder. See [Auto models and the model picker](/docs/user-documentation/agents/model-selection).
...
The lists below match the model options shown by the model-tier information panel as of September 1, 2026. They include the model and reasoning-effort combinations currently assigned to each tier. Asterisks mark models that depend on workspace configuration, plan eligibility, provider access, or another special rollout condition. Dust can change these assignments as providers release models, update pricing, or change availability.

*Some starred models may require an enabled workspace feature or an eligible plan. Claude Fable 5 requires additional provider-side configuration and is not guaranteed to be available in every workspace.*
...
| GPT-5 Mini | Light, Medium, High |
| GPT-5 Nano | Light, Medium, High |
...
| GPT 5 | Light, Medium, High |
| GPT 5.1 | None, Light, Medium, High |
| GPT 5.2 | None, Light, Medium, High |
| GPT 5.4 | None, Light, Medium, High |
...
| Claude Opus 4.7* | Light, Medium, High |
