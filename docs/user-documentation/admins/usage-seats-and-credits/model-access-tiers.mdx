...
| --- | --- |
| **Cost efficient** | Models that trade some performance for a much lower price, while remaining usable for everyday Dust work. |
...
### Do I have to configure anything for members to use the model picker?
...
It depends on the model and the reasoning effort. As a rule of thumb: small and mini models are Cost efficient, mid-size models are Cost efficient or Balanced at low effort and Premium at their highest effort, and frontier models are Premium at every effort. The exact tier is shown next to each model in the picker, and the tiers evolve as providers release new models and change prices.
...
Fast and Standard stay available. Complex is a premium selection: it is locked on legacy plans, and members whose ceiling sits below it cannot select it.
