...
To enable this, go to **Admin > Usage > Settings** and turn on the **Upgrade request** toggle. Once enabled, blocked members see a prompt to request an upgrade. Admins can then review and act on pending requests from the **Requests** tab in the Members section.
