...
The simplest way to get this right without auditing every agent is to use [auto models](/docs/user-documentation/agents/model-selection): **Fast**, **Standard**, and **Complex** route each request to a model sized for the task instead of defaulting to the most expensive model available.
