...
The simplest way to get this right without tracking individual models is to use [auto models](/docs/user-documentation/agents/model-selection#auto-models). **Fast**, **Standard**, and **Complex** are three ordered lists of models maintained by Dust at three price and capability levels. You choose the level per agent, and Dust keeps the list behind it current, instead of every agent defaulting to the most expensive model available.
