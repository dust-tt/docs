...
Managers cannot enable or disable Frames or audit logs. When Frames is enabled, Managers can configure which groups may invite users to Frames and publish Frames.
...
| Publish skills | Yes | When granted | When granted |
| Access unpublished agents | Yes | When granted | When granted |
...
| Invite users to Frames | Yes | When granted, if Frames is enabled | When granted, if Frames is enabled |
| Publish Frames | Yes | When granted, if Frames is enabled | When granted, if Frames is enabled |
...
The Frames feature toggle controls whether Frames is available in the workspace. When Frames is disabled, no one can use Frames, regardless of their group permissions.

The permissions to invite users to Frames and publish Frames are separate controls. They appear only when Frames is enabled.
