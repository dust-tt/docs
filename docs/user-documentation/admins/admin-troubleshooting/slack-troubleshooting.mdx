...
**Why this might not work:** Auto‑reply requires configuration and may have availability restrictions depending on your plan.

**Options:**

1. **Agent settings:** Configure a default agent for specific public channels in the agent's settings page ([Dust in Slack](/docs/user-documentation/agents/integrations/dust-in-slack/slack-workflows)).
...
**If you need bot identity:** A workspace admin can enable the **Slack Bot** tool under **Administration** > **Tools**, then attach it to the relevant Space under **Spaces** > **Tools**, and add it to agents in the Agent Builder. The bot posts as a bot identity instead of you, and only works in channels where the bot has been invited.
