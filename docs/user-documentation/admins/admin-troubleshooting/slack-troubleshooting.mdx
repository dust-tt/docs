...
<Warning>
**Workflows require workspace access**

On credit-priced plans, a workspace admin can enable a workflow from **Admin** > **Automations** > **Slack workflows**. The admin must enter the exact sender name shown in Slack and select the Spaces the workflow can reach. The Dust Slack bot must also be connected to the workspace.

The self-serve tab is not shown on legacy plans. Those workspaces continue using the existing support or Poke workflow for access management.
</Warning>
