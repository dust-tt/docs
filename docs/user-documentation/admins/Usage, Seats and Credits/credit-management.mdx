...
The workspace credit pool is managed by workspace admins and members with the **`Billing access`** capability:
...
By default, all members with workspace pool access can draw from it without individual limit. Workspace admins and members with the **`Billing access`** capability can configure a **personal spend limit** for each member to cap how much of the shared pool they can consume.
...
* Workspace admins and members with the `Billing access` capability can set a workspace-wide default limit. When the default is updated, only members currently on the previous default are affected.

Members can view their personal spend limit in the Usage page (**Workspace > Usage**).
...
Workspace admins and members with the **`Billing access`** capability can configure a monthly cap on programmatic credit consumption from **Workspace > Usage**. An email notification is sent when 80% of the cap is reached.

## Tracking usage

Workspace admins and members with the **`Billing access`** capability can manage credit settings—including the workspace pool, spending limits, and programmatic caps—from **Workspace > Usage**. **Managers** can also access this page to view usage data, edit member seat types, and review upgrade requests.

## Workspace credit pool

At the top of the page, the current workspace credit pool balance is displayed (credits consumed vs. total available). Workspace admins and members with the `Billing access` capability on Business plans can purchase top-up packages directly from this page using the **Top up** button.
...
The Members section lists all workspace members with their usage details. **Admins** and **Managers** can:
...
In addition, workspace admins and members with the **`Billing access`** capability can adjust a member's personal spend limit from the same **···** menu.
