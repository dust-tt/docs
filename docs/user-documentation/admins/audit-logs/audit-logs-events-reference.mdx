...
| Event | Description |
|---|---|
| `skill.self_improvement_updated` | A skill's self-improvement setting was updated |
...
| `membership.bulk_role_updated` | Multiple members' roles were changed in bulk |
...
| Event | Description |
|---|---|
| `group.advanced_model_access_updated` | A group's advanced model access setting was updated |
| `group.spend_limit_updated` | A group's spend limit was updated |
...
| `workspace.audit_logs_updated` | The workspace's audit log collection setting was changed |
| `workspace.governance_permission_updated` | A governance capability's configuration was changed. Metadata: `grant_type` (the capability identifier), `resource_type`, `scope` (the new mode: `everyone`, `groups`, or `admin_only`), `group_ids` (affected group IDs when scope is `groups`) |
