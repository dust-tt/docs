...
Audit Logs are available to **workspace admins** and to users who have been granted the **`Access audit logs`** governance capability. Navigate to:
...
<Note>
  The Audit Logs toggle (enabling or disabling audit log collection for the workspace) is configured under **Admin > Settings & Governance** and requires the **Admin** role.
</Note>

## What is logged

Audit Logs record all significant workspace actions, including **governance changes**: role assignments and governance capability configuration changes are logged as `workspace.governance_permission_updated`. See the [Events Reference](/docs/user-documentation/admins/audit-logs/audit-logs-events-reference) for the full list.
