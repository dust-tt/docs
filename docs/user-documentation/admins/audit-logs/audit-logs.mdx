...
Audit Logs are available to **Admins** and to users who have been granted the **`Access audit logs`** governance capability. Navigate to:
...
<Note>
  The Audit Logs toggle (enabling or disabling audit log collection for the workspace) is configured under **Admin > Workspace & Governance > Governance** and requires the **Admin** role.
</Note>
