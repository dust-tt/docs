---
title: "Analytics"
description: "Monitor workspace credit consumption, identify what drives it, and export raw analytics data."
---
...
- **Last 180 days**: a rolling 180-day window.

The period applies to the summary, chart, attribution table, comparisons, and raw-data export.

Use the granularity selector next to the period selector to group the consumption chart by **Daily**, **Weekly**, or **Monthly** buckets. Changing granularity changes the chart's time buckets without changing the selected period.
...
Cap status and cap values do not appear for the rolling seven-day, 30-day, 90-day, and 180-day periods.

![Analytics overview showing the selected cycle, active members, data freshness, consumption status, credits used, and top agent.](/images/analytics/analytics-overview.png)
...
### Find spikes with the per-period view

**Per period** displays credits consumed in the selected granularity as stacked bars. With **Daily**, **Weekly**, or **Monthly** granularity, each bar represents the corresponding day, week, or month. The bars are broken down using the dimension selected in Attribution. For example, selecting **Models** in Attribution changes the chart from an agent breakdown to a model breakdown.

The chart displays the five highest-consuming contributors individually. **Others** combines the remaining contributors. Hover over a bar to see the credits attributed to each contributor and the total for that period.

When **Daily** granularity is selected, the **Today (partial)** marker identifies the current, incomplete day. Its bar is faded because today's usage is still being added, so the total may increase. Avoid comparing it directly with completed days.
...
<Note>
Each value in a filter category is checked for activity given the selected period and the values already applied to the other categories. A value with no matching activity in that context is marked **No activity**. It stays visible so you can see what exists in the workspace, but you cannot select it until another filter change makes it available again. If a value you already selected later shows **No activity** because of a change elsewhere, it remains selected and you can still remove it.
</Note>
...
The selected Attribution tab also controls the breakdown used by the **per-period** chart.
...
![Attribution section showing dimension tabs, raw-data export, search, consumption share, total credits, credits per message, previous-period comparisons, and row controls.](/images/analytics/analytics-attribution.png)
...
| **Others** | Consumption outside the five highest-consuming contributors shown separately in the per-period chart |
