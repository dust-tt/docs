...
- The number of active members out of all workspace members. An active member is a member associated with consumption during the selected period.
- When the latest consumption record was indexed. Analytics data is not real-time, so recent activity may take time to appear.
...
The **Today (partial)** marker identifies the current, incomplete day. Its bar is faded because more consumption may still be indexed. Avoid comparing it directly with completed days.

![Daily consumption chart showing filters, daily and cumulative controls, stacked agent consumption, the legend, and the Today partial marker.](/images/analytics/analytics-consumption.png)
...
<Note>
Analytics includes consumption from agents marked **Not published**, including agents you do not have access to as an editor. This gives admins and managers a complete view of workspace consumption without exposing the agent's description. For an inaccessible not-published agent, Analytics shows the email address of the member who created it instead.
</Note>
...
- **Credits / message**: average credits per message for agents, members, groups, models, sources, and API keys.
- **Credits / invocation**: average credits per invocation for tools and skills.
- **Vs Prev**: the change from the immediately preceding period of the same length. For an in-progress cycle, Analytics compares the elapsed part of the cycle with an equally long preceding window. `--` appears when there is not enough previous data to calculate a percentage.
...
| **Active member** | A workspace member associated with consumption during the selected period |
| **Consumption share** | A contributor's credits divided by all credits in the selected scope |
| **Credits / message** | Average credits consumed per distinct message |
| **Credits / invocation** | Average credits consumed per tool or skill invocation |
| **Vs Prev** | Credit change against the immediately preceding period of equal length |
